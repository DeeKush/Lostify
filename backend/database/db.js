...
const userDb = {
  ...
  toggleEnabled: (id) => {
    ...
    db.prepare('UPDATE users SET isEnabled = ? WHERE id = ?').run(newStatus, id);
    return userDb.getById(id);
  }
};
...
const postDb = {
  ...
  getByUser: (userId) => {
    return db.prepare('SELECT * FROM posts WHERE user = ? ORDER BY createdAt DESC').all(userId);
  },
  ...
  getActive: () => {
    return db.prepare("SELECT * FROM posts WHERE status = 'active' ORDER BY createdAt DESC").all();
  },

  getResolved: () => {
    return db.prepare("SELECT * FROM posts WHERE status = 'resolved' ORDER BY createdAt DESC").all();
  }
};
...
const feedbackDb = {
  ...
  getPendingCount: () => {
    const result = db.prepare("SELECT COUNT(*) as count FROM feedback WHERE status = 'pending'").get();
    return result.count;
  }
};
...
const settingsDb = {
    ...
    if (existing) {
      ...
      Object.keys(settings).forEach(key => {
        if (settings[key] !== undefined) {
          fields.push(`${key} = ?`);
          values.push(typeof settings[key] === 'boolean' ? (settings[key] ? 1 : 0) : settings[key]);
        }
      });
      ...
      const stmt = db.prepare(`UPDATE settings SET ${fields.join(', ')} WHERE userId = ?`);
      stmt.run(...values);
    } else {
      const id = generateId();
      const stmt = db.prepare(`
      ...
      stmt.run(
        ...
        settings.whatsappPrefix !== undefined ? (settings.whatsappPrefix ? 1 : 0) : 1,
  ...
  setAdminSettings: (settings) => {
    ...
    const stmt = db.prepare(`UPDATE admin_settings SET ${fields.join(', ')} WHERE id = 1`);
    stmt.run(...values);
